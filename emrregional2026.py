...
class ClienteReniecOnpeAPI:
    ...
    def obtener_datos_adicionales(self, dni: str, provincia: str) -> Dict[str, str]:
        ...
        idx = int(dni_formatted) % len(locales) if dni_formatted.isdigit() else 0
        local_obj = locales[idx]
        num_val = int(dni_formatted) if dni_formatted.isdigit() else 100
        mesa = f"0{num_val % 900 + 1000}"
        aula = f"Aula {num_val % 20 + 101}"
        return {
            "dni": dni_formatted,
            "direccion": f"Calle Principal #{num_val % 300 + 10}, {local_obj['distrito']}",
...
try:
    ...
    # MÓDULO 3: GEOLOCALIZACIÓN Y MAPAS
        ...
        if HAS_FOLIUM:
            ...
            st.warning("⚠️ El módulo de mapas requiere que `folium` y `streamlit-folium` estén agregados en tu archivo `requirements.txt`.")
